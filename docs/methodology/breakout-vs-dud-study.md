...
4. **A surprising number is a prompt to check a variable, not to publish.** We decoded video age from the TikTok id and confirmed the recency confound before believing the 41%. The scaled clean read gives 83%.
...
An earlier 48-pair diagnostic broke predictions out by niche (the concept-matched pairs carry a niche label; the same-creator pairs are niche-unlabeled in the corpus and sit as one mixed block):
...
First on an older 18-pair same-creator diagnostic: laws 1+2+5 only (visual question, payoff taste, social/emotional charge) scored 9/18 = 50%, exactly chance, while laws 3+4 only scored 12/18 = 67%. We then ran the laws-3+4-only condition across the older 48-pair diagnostic slice:

| Tier | Laws 3+4 only, older diagnostic |
|---|---|
| Extremes (~1000x gap) | 14/18 = 78% |
| Closer-margin (8 to 34x) | 8/12 = 67% |
| Same-creator (clean) | 12/18 = 67% |
| Pooled | 34/48 = 71% (CI 57 to 82%, z=2.89) |

Reading it honestly: laws 3 (per-frame novelty) and 4 (no format label) are the load-bearing pair in the older ablation diagnostic. Treat that diagnostic as directional and read the scaled full-five headline table above for the current tier results. Product takeaway: lead the generation gate with laws 3 and 4, but keep all five.

...
- **Headline:** "An AI agent that doomscrolls for you."
- **Positioning line:** "Delegate the ideation. Oversee the rest."
...
- **Audience:** founders, e-commerce operators, and creators who already live in Claude Code / Codex.
...
2. **Hero (centered).** Mono eyebrow, H1 with one italic-serif highlighted word, confident subhead, **two CTAs max** (primary "Get started", text-link "See it work"), compatibility line ("Works with Claude Code & Codex"), and an **animated terminal** running a single `/abg` command with streaming success lines. No switcher.

...
- **`mcp-install.html`** — the separate install page (lazyweb `/mcp-install` style). Generates a free install token, has Claude Code / Cursor / Codex tabs, and produces the paste-ready prompt that installs the MCP. Landing-page "Get started" / "Get the MCP" CTAs link here.

...
An AI agent that doomscrolls for you.

Software has spent forty years being built for a human clicking around a screen. That era is ending. The agent is the customer now, and LazyReel is built for it: this is the beginning of agent-first marketing, where you hand the whole ideation phase to a system that scrapes and remembers far more than you ever could.

LazyReel is a free MCP server that already doomscrolled 21B+ views of short-form video (and app ads) so your agent doesn't start from zero. Plug it in and your agent knows what is breaking out in your niche: the hooks, the formats, and the real videos behind them, before it writes a single line. You delegate the whole ideation phase, what is worth making and why; it hands back the decoded research and the brief.

This is the taste layer, the brain. Generation tools like Higgsfield are excellent hands, they will render anything, but they do not tell you what to make. LazyReel decides what is worth making and hands over the exact brief; the gen MCP renders it. Same agent, same chat. You stop shipping slop because what you generate is grounded in what actually broke out.

It is the first layer of an agent-first marketing stack. Decode with evidence, not vibes.
...
The easy way is the install page. It generates a free token and gives you one prompt to paste into your agent. Open `mcp-install.html`, copy the block, paste it into Claude Code, Codex, or any MCP client, and you are done.
...
For Codex or any other MCP client, run the same build, then point the client at the printed path of `build/index.js`. The install page writes the exact config.

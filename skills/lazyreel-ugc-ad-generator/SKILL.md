...
- **Length is not capped.** The shot count follows the script and the target length, not the preset minimum. A 12-15s ad is a handful of shots; a 30-90s video is many. To go long, expand the angle into more beats (more escalation/demo shots before the payoff) and lean on the reference-to-video chain below so one creator, wardrobe, and product hold across every shot. The pipeline (generate each shot, then stitch) is identical at any length, the manifest just has more shots.

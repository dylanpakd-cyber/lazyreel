...
3. **Per-frame novelty.** This is why the output is multiple clips, not one (as many as the length needs; more clips for a longer video). Each clip is a distinct framing (wide, macro, reframe, insert). Prompt macro and motion.

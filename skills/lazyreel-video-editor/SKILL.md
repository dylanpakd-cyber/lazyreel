...
description: Assemble and cut UGC into a finished video that wins, at any length. Takes the generated clips (any number, from a few for a short ad to dozens for a 60-90 second-plus video) from a video-gen MCP (Higgsfield, Seedance) or raw UGC footage and edits them into a fast-cut, sound-off-legible, hook-first 9:16 piece that obeys the validated breakout laws. Real FFmpeg and Remotion, not hand-waving. Use when the user has clips to stitch, wants to cut footage into a short or a long video, asks to "edit the video," "stitch these clips," "add captions," "make it 9:16," or hands off rendered clips from the director skills. Pairs with the LazyReel MCP (breakout_laws for the cut rhythm) and the director skills upstream.
---

# LazyReel video editor

The cut is half the video. A video-gen MCP gives you clips; the edit is what makes them a breakout. The single biggest mistake is shipping one continuous clip. Real video that travels is **many clips pieced together with hard cuts**, because per-frame novelty is one of the two strongest things we measured (see `references/cut-rhythm.md`). The clip count follows the length: a 12-15s ad is a handful, a 60-90s video is dozens. The cut rhythm (a cut every ~1.5 to 3s) is the same at any length; a longer video is simply more clips on the timeline. Your job is to assemble those clips into a piece whose first 3 seconds earn the watch and whose cut rhythm holds it the whole way.
...
A video is **many clips**, each a distinct shot or angle, cut hard, however many the length needs (a 10-15s ad is ~3 to 5, a 60s video is 12 to 18+). One static clip dies on per-frame novelty (law 3) and reads as AI. If the director only produced one clip, send it back for the full sequence, or cut the one clip into multiple framings (push-in, reframe, macro insert) so the edit still changes every few seconds.
...
- **Middle clips: escalation.** Each a new framing or beat, as many as the script has. Product enters here as a helper, not the star. Cut every 1.5 to 3 seconds.
- **Final clip: the payoff** the hook teased, then a hard end. No drawn-out CTA.

For a long video the plan is the same with more middle clips; keep the first third dense, and let the energy arc (not the length) decide where a beat can hold a little longer. The concat list below just grows, the pipeline does not change.
...
- [ ] As many clips as the length needs (more for longer videos), never one static clip
